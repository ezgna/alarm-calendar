  ...
  return (
    <View className={`flex-1 items-center justify-center px-6 ${t.appBg}`}>
      <View className="w-full max-w-md items-center gap-4">
        <Text className={`text-lg font-semibold text-center ${t.text}`}>プレミアム登録のお願い</Text>
        <Text className={`text-sm text-center ${t.textMuted}`}>
          神アラームのご利用には、月額プラン（30日間の無料お試しを含む）の登録が必要です。登録が完了するまで、アプリ本体の機能は利用できません。
        </Text>
        ...
        <View className="w-full mt-4">
          <TouchableOpacity
            ...
            disabled={busy}
            className={`px-4 py-3 rounded-full items-center ${t.buttonPrimaryBg}`}
          >
        ...
        <Text className={`mt-4 text-[11px] text-center ${t.textMuted}`}>
          いつでも解約可能です。料金や解約方法の詳細は、App Store のサブスクリプション管理画面をご確認ください。
        </Text>
      </View>
    </View>
  );
